...
priordate = uk['Date'][(uk.Date > '2008-10-04') & (uk.Date <= '2008-11-04')]
priorstock = uk['Stock'][(uk.Date > '2008-10-04') & (uk.Date <= '2008-11-04')]
priorclose = uk['Close'][(uk.Date > '2008-10-04') & (uk.Date <= '2008-11-04')]
...
postdate = uk['Date'][(uk.Date > '2008-11-04') & (uk.Date <= '2008-12-04')]
poststock = uk['Stock'][(uk.Date > '2008-11-04') & (uk.Date <= '2008-12-04')]
postclose = uk['Close'][(uk.Date > '2008-11-04') & (uk.Date <= '2008-12-04')]
...
CPI.plot(x='Month',y='cpi',title='UK CPI Trend',legend = False)

# Apply inflation rate:
octcpi = float(CPI.cpi[CPI.Month == '2008 OCT'])
novcpi = float(CPI.cpi[CPI.Month == '2008 NOV'])
...
plotdata = pd.DataFrame({'Stock':plotx,
                         'Diff':stockdiff})
plotdata.plot.scatter(x='Stock',y='Diff',title='2008 Presidential Election (2008.11.04) Effect on UK Market') 
...
volumedata = pd.DataFrame({'Date':voldate,
                           'TotalVolume':vol})
volumedata.plot.bar(x='Date', y = 'TotalVolume', title = '2008 Presidential Election (2008.11.04) Effect on UK Market')
